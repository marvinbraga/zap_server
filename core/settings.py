...
ELEMENTS_DATA_FILE = os.path.normpath(os.path.join(f'{BASE_DIR}/whatsapp/data/', 'elements_data.pkl'))
SETTINGS_TYPE = os.environ.get('SETTINGS_TYPE', default='PRD')

OS_WEB_DRIVER = platform.system()
if OS_WEB_DRIVER == 'Windows':
    CHROME_WEB_DRIVER = os.path.join(f"{BASE_DIR}\\whatsapp\\lib\\win\\", 'chromedriver.exe')
    FIREFOX_WEB_DRIVER = os.path.join(f"{BASE_DIR}\\whatsapp\\lib\\win\\", 'geckodriver.exe')
else:
    CHROME_WEB_DRIVER = os.path.normpath(os.path.join(f"{BASE_DIR}/whatsapp/lib/", 'chromedriver'))
    FIREFOX_WEB_DRIVER = os.path.normpath(os.path.join(f"{BASE_DIR}/whatsapp/lib/", 'geckodriver'))
